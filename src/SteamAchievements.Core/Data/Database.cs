...
public static class Database
{
    ...
    /// <summary>
    /// Deletes every row that belongs to the previous account: games,
    /// achievements, sync history, and the stored identity in <c>settings</c>.
    ///
    ...
    ///
    /// Kept apart from <see cref="Vacuum"/> — not folded into one method — so a
    /// caller that wants the two timed separately, like
    /// <see cref="SqliteLibraryReset.Reset"/>, can. <c>Database</c> itself has
    /// no logger anywhere else; the timing belongs at that seam, not here.
    /// </summary>
    public static void EmptyLibrary(SqliteConnection connection)
    {
    ...
    /// <summary>
    /// Reclaims the space <see cref="EmptyLibrary"/>'s deleted rows occupied —
    /// for a 1500-game library, most of the file.
    ///
    /// Cannot run inside a transaction, which is why it is a separate method
    /// rather than folded into <see cref="EmptyLibrary"/>. Callers that care
    /// about the timing, like <see cref="SqliteLibraryReset.Reset"/>, measure
    /// it on its own: this is the one statement whose behaviour with three live
    /// connections against a WAL database has never been observed.
    /// </summary>
    public static void Vacuum(SqliteConnection connection) => connection.Execute("VACUUM");
    ...
    ///
    /// A convenience over <see cref="EmptyLibrary"/> and <see cref="Vacuum"/>
    /// for callers — most of the test suite — that do not need the two timed
    /// separately. <see cref="SqliteLibraryReset.Reset"/> calls the two
    /// directly instead, because it does.
    /// </summary>
    public static void ResetLibrary(SqliteConnection connection)
    {
        EmptyLibrary(connection);
        Vacuum(connection);
    }
