...
2. **API key — a single paste.** A button opens the browser on the key
   issuance page, where the user is already signed in; the user pastes the key
   back into a field, and the app checks it against Steam before storing it.

   Clipboard watching was specified here originally — the app filling the key
   in by itself once it saw 32 hex characters — and was dropped during the
   Windows host design (see `docs/superpowers/specs/2026-07-26-windows-host-design.md`
   §3.4). It bought one click and cost a background clipboard reader, a window
   message hook and P/Invoke in the one project that cannot be tested on
   macOS.
